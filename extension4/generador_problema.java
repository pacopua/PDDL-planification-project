...
public class generador_problema {
    ...
    public static void main(String[] args) {
        ...
        while (repetir) {
            try {
                System.out.println("Elija el numero minimo y maximo de minutos por contenido\n" + "minimo: ");
                minMinutos = Integer.parseInt(System.console().readLine());
                System.out.println("maximo: ");
                maxMinutos = Integer.parseInt(System.console().readLine());
                repetir = false;
            } catch (NumberFormatException e) {
                System.out.println("Por favor, introduzca un numero valido");
            }
        ...
        //Pair<Integer, Integer> dia = new Pair<>(minDias, maxDias);
        //Pair<Integer, Integer> contenido = new Pair<>(minContenidos, maxContenidos);
        generarPDDL("extension4/random_problem1.pddl", minDias, maxDias, minContenidos, maxContenidos, minMinutos, maxMinutos);
    }

    public static void
    generarPDDL(String filename, int minDia, int maxDia, int minContenido, int maxContenido, int minMinutos, int maxMinutos) {
        int numDays = random.ints(minDia, maxDia + 1).findFirst().getAsInt();
        int numContents = random.ints(minContenido, maxContenido + 1).findFirst().getAsInt();
        ...
        try (FileWriter file = new FileWriter(filename)) {
            file.write("(define (problem PROBLEMA_EXTENSION4)\n");
            file.write("    (:domain DOMAIN_EXTENSION4)\n");
            file.write("    (:objects\n");
            for (String content : contents) {
                file.write("        " + content + " - contenido\n");
            }
            ...
            for (int i = 0; i < numContents - 1; i++) {
                for (int j = i + 1; j < numContents; j++) {
                    ...
                    if (random_float > 0.80) { // 20% de probabilidad de que haya un predecesor
                        file.write("        (predecesor " + contents.get(i) + " " + contents.get(j) + ")\n");
            ...
            for (int i = 0; i < numContents - 1; i++) {
                for (int j = i + 1; j < numContents; j++) {
                    // 0.0 <= random_float < 1.0
                    float random_float = random.nextFloat( (float) 1.0);
                    if (random_float > 0.80 && !paralelos[i][j] && !predecesor_de[i][j]) {
                    /* 20% de probabilidad de que haya un paralelo, ademas
            ...
            int z = 1;
            for (String day : days) {
                file.write("        (= (minutosAgendados " + day + ") 0)\n");
                file.write("        (= (orden " + day + ") " + z + ")\n");
                ++z;
                //"(= (contenidosAgendados lunes) 0)"
                //file.write("        (not (lleno " + day + "))\n");
            ...
            for (String content : contents) {
                file.write("        (= (duracion " + content + ") " + random.ints(minMinutos, maxMinutos + 1).findFirst().getAsInt() + ")\n");
            }
